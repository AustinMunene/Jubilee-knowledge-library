...
export default function BookEditForm({ book, onSaved }: { book: any; onSaved?: (b: any) => void }) {
  ...
  function validateForm(): boolean {
    ...
    if (total < (book.available_copies || 0)) {
      newErrors.total = `Total copies cannot be less than available copies (${book.available_copies || 0})`
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-slate-800/50 border border-slate-700 p-6 rounded-lg space-y-4">
      ...
      <div>
        ...
        {book.cover_url && !useFileUpload && !coverUrl && (
          <div className="mb-2">
            <img src={book.cover_url} alt={book.title} className="w-24 h-32 object-cover rounded-lg border border-slate-700" />
          </div>
        )}
        <div className="mb-3">
          <div className="flex gap-4 mb-3">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-slate-300">Upload New Image</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="radio"
